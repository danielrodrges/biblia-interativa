...
import { signUpWithEmail, signInWithGoogle, signInWithFacebook } from '@/lib/supabase';
import { Mail, Lock, User, AlertCircle, CheckCircle, Book } from 'lucide-react';
...
export default function SignUpPage() {
  ...
  const handleFacebookSignUp = async () => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao cadastrar com Facebook.');
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F6] flex items-center justify-center px-6 py-8">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-3xl shadow-xl shadow-stone-200/50 p-8 text-center border border-stone-100">
            <div className="w-20 h-20 bg-stone-100 rounded-3xl flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="w-10 h-10 text-stone-800" />
            </div>
            ...
            <Link
              href="/auth/login"
              className="inline-block bg-stone-900 text-stone-50 px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-stone-800 hover:shadow-xl transition-all"
            >
              Ir para Login
            </Link>
          </div>
    ...
    <div className="min-h-screen bg-[#FAF9F6] flex items-center justify-center px-6 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-stone-200/50 p-8 mb-6 border border-stone-100 animate-in slide-in-from-bottom-8 fade-in duration-700 delay-150">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-stone-500">Ou cadastre-se com</span>
            </div>
          </div>
        ...
        <div className="text-center">
          <p className="text-stone-600">
            Já tem uma conta?{' '}
            <Link href="/auth/login" className="text-stone-800 font-bold hover:underline">
              Entrar
            </Link>
          </p>
        </div>
